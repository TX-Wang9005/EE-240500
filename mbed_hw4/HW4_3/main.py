...
while(True):
   ...
   for tag in img.find_apriltags(fx=f_x, fy=f_y, cx=c_x, cy=c_y): # defaults to TAG36H11
      ...
      print(("%f %f %f %f %f %f\n" % print_args).encode())
      if (print_args[4] > 358 or print_args[4] < 50):
        uart.write(("/turn/run -2 0.1\n").encode())
        print("right")
      elif (270 < print_args[4] < 355):
        uart.write(("/turn/run 5 0.1\n").encode())
        print("left")
      else:
        uart.write(("/stop/run \n").encode())
        uart.write(("/angle/run \n").encode())
        print("stop")
      #uart.write(("/stop/run").encode())
      time.sleep(1)
